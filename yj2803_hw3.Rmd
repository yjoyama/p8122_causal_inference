...
g-formula for the randomized study:  $E[Ya] = \sum_c E[Y | A = a, C = c] Pr(C = c)=E[Y|A=a]$  
g-formula for the observational study: $E[Ya] =  \sum_c E[Y |A=a, C = c] Pr(C = c)$ under NUCA.  
Randomization enforces unconfoundedness. The distribution of $C$ does not depend on $A$, so g-formula simplifies to $E[Y|A=a]$. In an observational study, $A$ is influenced by $C$, so the formula cannot be simplified without accounting for the confounders.   
