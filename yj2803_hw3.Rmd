...
The 95% confidence interval is (`r list(round(ci_lower, 2), round(ci_upper, 2))`).  
The point estimate suggests that the outcome \( Y \) is `r round(diff,2)` mg/dL higher on average in the bright light group compared to the dark light group among non-obese mice in this observational study. We cannot draw inferences about the causal effect in obese mice due to the violation of probabilistic assumptions. Since the 95% confidence interval does not includes zero, the point estimate is statistically significant at the 95% confidence level.  
