...
df_trim |> 
  ...
  mutate(
    ...
    varace = sum((prop^2)*(var.Y.0/n.0 + var.Y.1/n.1)), # variance of a stratified estimator for a weighted mean difference across strata
...
8. In order to compare it with subclassification approach, I will use trimmed data to construct linear regression model. 
```{r}
# linear model
df_trim |> 
  ...
  kable()
```
From the results, we can see that `treat = 1` has $822.05 more income than `treat = 0`, holding other variables constant. However, the p-value >0.05 indicates that this is not statistically significant. Although the point estimate is quite different from what we observed in the subclassification approach, both are consistent with the insignificant effect of the job training program on income in 1978.
